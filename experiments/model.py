...
class Model:
  ...
  @torch.enable_grad()
  def backprop(self, theta=None, labelflipping=False, numb_labels=None, 
              mvr=False, previous_params=None, dataset_worker=False, batch_increase=False, original_batch_size=None,
  ...
  @torch.no_grad()
  def train_loss(self, honest_theta, other_workers, dataset=None, loss=None, nb_batch_per_worker=1):
    """Estimate the train loss at the parameter of honest_node"""
    ...
    for worker_id in other_workers:
        for _ in range(nb_batch_per_worker):
          inputs, targets = dataset.sample_worker(worker_id, self._config)
          res += loss(self.run(inputs), targets, self._params)
    return res/nb_batch_per_worker / len(other_workers)
