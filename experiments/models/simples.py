...
__all__ = ["full", "small", "fullTan", "little", "conv", "logit", "logitm", "linear", "jaggi", "minimalcnn", "smallCNN_cifar"]
...
class _Little(torch.nn.Module):
	...
	def __init__(self):
		...
		self._f1 = torch.nn.Linear(28 * 28, 128)
		self.bn1 = torch.nn.BatchNorm1d(128)
		self._f2 = torch.nn.Linear(128, 10)
	...
	def forward(self, x):
		...
		# Forward pass
		x = x.view(-1, 28*28)
		x = torch.nn.functional.relu(self.bn1(self._f1(x)))
...
# ---------------------------------------------------------------------------- #
# "Simple" convolutional model, for MNIST - huuuge number of parameters wtf
...
class _MinimalCNN(torch.nn.Module):
    def __init__(self):
        super(_MinimalCNN, self).__init__()
...
def minimalcnn(*args, **kwargs):
	...
	"""
	global _MinimalCNN
	return _MinimalCNN(*args, **kwargs)
...
def smallCNN_cifar(*args, **kwargs):
	""" Build a new simple, convolutional model.
	Args:
	... Forwarded (keyword-)arguments
	Returns:
