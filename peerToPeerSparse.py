...
with tools.Context("cmdline", "info"):
	...
	# Check the number of nodes on which test is evaluated
	if args.subsampled_evaluation <=0:
		args.subsampled_evaluation = args.nb_honests
	...
	if args.learning_rate_schedule is None:
		...
		lr_schedule = tuple(float(number) if cnt % 2 == 0 else int(number) \
					  for cnt, number in enumerate(args.learning_rate_schedule.split(",")))
	...
	# Update the clipping parameter if coordinate descent is used: multiply it by sqrt(d'/d)
    ...
    # Set corresponding parameters if learning in the jungle is chosen
	if args.jungle:
		args.batch_size = 1
...
with tools.Context("setup", "info"):
	...
	network = topology.create_graph(
		...
		weights_method=args.topology_weights, seed=args.seed)	# Configurations
	# Configurations
	...
	# Number of nodes on which the accuracy and losses are computed
		...
		try:
			...
			try:
				...
				if args.evaluation_delta > 0:
					result_make("eval", "Step number", "Cross-accuracy", "Average loss")
				# Make study file
				if args.nb_for_study > 0:
...
#### Writing here how to get a node neighbors.
...
with tools.Context("training", "info"):
	...
	while not exit_is_requested():
		...
		if milestone_evaluation:
			...
			res_acc = 0
			samples_acc = torch.randperm(args.nb_honests, dtype=int, device='cpu')[:args.subsampled_evaluation].tolist()
			nb_samples=len(samples_acc)
			for i in samples_acc:
				# Evaluate the test accuracy of the model of worker i
				res_acc += model.eval(honest_theta=honest_thetas[int(i)])
				for _ in range(args.batch_size_test_reps - 1):
					res_acc += model.eval(honest_theta=honest_thetas[int(i)])
				
				# Evaluate the train loss of model of worker i
				res_loss += model.train_loss(honest_theta=honest_thetas[int(i)], 
								 other_workers=torch.randperm(args.nb_honests, dtype=int, device='cpu')[:args.subsampled_evaluation].tolist(), 
								 nb_batch_per_worker=1).item()

			acc = res_acc[0].item() / res_acc[1].item()
			res_loss = res_loss/nb_samples

            #JS: Calculate the accuracy over time
			#acc = (acc * (nb_milestones - 1) + acc_new) / nb_milestones
			print(f" acc {acc * 100.:.2f}%." + f" loss {res_loss :.2f}")
			# Store the evaluation result
			if fd_eval is not None:
				result_store(fd_eval, steps, acc, res_loss)
		...
		for honest_worker in network.nodes:	
			...
			# selecting only parameters of neighboring nodes, and the one of "honest_worker"
			
			indices = network.nodes # network.neighbors(honest_worker)) + [honest_worker]
			correct_thetas = [honest_thetas[k] for k in indices]
			
			# We compute the weights of the 
